// V1 Inbox to Calendar — Phase 2 — ingester.gmail.ts
// Ingest 60 days of Gmail messages + attachments into messages_v1 / attachments_v1.
...
// Ensure the v1-attachments bucket exists
...
async function ensureV1Bucket(): Promise<void> {
  ...
  if (!exists) {
    ...
    if (error) {
      console.error('[v1-gmail] bucket creation error:', error.message);
    } else {
      console.log('[v1-gmail] bucket v1-attachments created');
    }
  }
  _v1BucketReady = true;
}
...
async function uploadV1Attachment(
  ...
  try {
    ...
    if (error) {
      console.error('[v1-gmail] upload error:', error.message);
      return null;
    }
    ...
    console.error('[v1-gmail] uploadV1Attachment error:', err.message);
    return null;
  }
}
...
async function extractText(
  ...
  if (isPdf) {
    try {
      ...
      console.warn('[v1-gmail] pdf-parse error:', err.message);
  ...
  if (isDocx) {
    try {
      ...
      console.warn('[v1-gmail] mammoth error:', err.message);
      return { text: null, status: 'corrupt' };
    }
  }
...
export async function ingestGmail60d(userId: string): Promise<{
  ...
  // 1. Get refresh_token from configurations table
  ...
  if (configErr || !config?.refresh_token) {
    const msg = `[v1-gmail] No Gmail refresh_token for user ${userId}`;
    console.error(msg);
    errors.push(msg);
    return { messages_count, attachments_count, errors };
  }

  // 2. Build Gmail provider
  const provider = new GmailProvider({ refreshToken: config.refresh_token, userId });
  ...
  for await (const rawMsg of provider.listMessagesSince(after, MAX_MESSAGES)) {
    ...
    try {
      ...
      // The GmailProvider.getFullMessage returns body as plain text (already extracted).
      // We need raw HTML too — but the provider abstracts it. We use what we have.
      // body is already text from extractBody(). We apply forward chain cleaning.
      bodyText = cleanForwardChain(full.body);
      ...
      if (msgErr) {
        const e = `[v1-gmail] upsert messages_v1 error: ${msgErr.message} (msg ${rawMsg.id})`;
        console.error(e);
        errors.push(e);
        continue;
      }
      ...
      for (const att of full.attachments) {
        try {
          ...
          if (attErr) {
            const e = `[v1-gmail] insert attachments_v1 error: ${attErr.message} (${att.filename})`;
            console.error(e);
            errors.push(e);
          } else {
            attachments_count++;
          ...
          const e = `[v1-gmail] attachment processing error: ${attErr.message} (${att.filename})`;
          console.error(e);
          errors.push(e);
        }
      }
    } catch (msgErr: any) {
      const e = `[v1-gmail] message processing error: ${msgErr.message} (id ${rawMsg.id})`;
      console.error(e);
      errors.push(e);
    }
  }

  console.log(`[v1-gmail] ingest complete: ${messages_count} messages, ${attachments_count} attachments, ${errors.length} errors`);
  return { messages_count, attachments_count, errors };
}
